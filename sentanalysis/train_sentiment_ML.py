...
def predict(dirs: Dict[str, str]) -> None:
    """
...
def vectorize_and_save_ds(dirs: Dict[str, str], which_df: str = 'kaggleNOT') -> None:
    """
...
def create_model(dirs: Dict[str, str]) -> None:
    """
    Set the model architecture and train.
...
#===getting data
def get_kaggle_tweets(dataset_path: str, start: int = 0, amount: int = 0) -> pd.DataFrame:
...
def string_to_nparray(a_string: str) -> np.ndarray:
...
def split_datasets(data: pd.DataFrame, train_size: int = 1024*70, test_size: int = 2048
    ...
    data['tweet'] = data['tweet'].apply(string_to_nparray)
...
def tweet_model(): 
    ...
    model = Sequential()
    model.add(Input(shape=(512,),dtype='float32')) # 512 taken from example
...
def train(dirs, model, ds, val_ds):# train_ds, val_ds, steps_per_epoch=100):#embeded_tweets, targets):
    ...
    cp_callback = tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_path,
                                                 ...
                                                 verbose=1)
    history = model.fit(x = ds[0], y = ds[1],#train_ds,    # allows creating batches
                        epochs=num_epochs,
